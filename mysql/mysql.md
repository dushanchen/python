...
### 1.2 MySQL 支持哪些存储引擎?
    MySQL支持很多种存储引擎，最常用的就是InnoDB和MyISAM，此外还有Memory、Archive（只支持插入和查询）、CSV引擎 等。在绝大多数情况下，我们都会选择InnoDB，它现在也是MySQL的默认存储引擎了。

### 1.3 MySQL中MyISAM与InnoDB的区别是什么?
    MyISAM 不支持外键、不支持事务，只缓存索引，不缓存数据本身，只支持表级锁，不支持行锁，奔溃后无法安全恢复，适合读多写少的场景。
    ...
    InnoDB支持外键，而MyISAM不支持；MyISAM曾经在全文索引上有优势，不过现在新版的InnoDB也支持了。总的来说，现在InnoDB是更主流的选择，尤其是在对数据一致性和并发性要求高的业务里。
...
### 1.12 MySQL执行查询的过程
    ...
    记录语句底层执行过程的开关
    select @@profiling;  show variables like 'profiling';  默认OFF，不开启
    set @@profiling=1; 开启

    show profiles；  查看执行的sql
    show profile cpu,block io for query $id； 查看sql执行耗时
...
## 3.索引
    show variables like '%sotrage_engin%';
### 3.1索引的种类
    普通索引、唯一索引、复合索引
    聚簇索引、非聚簇索引
    B+tree索引、哈希索引、全文索引
...
### 3.2 哪些字段适合建索引
    1.主键、外键，主键自动建立索引，外键在外键约束声明情况下自动建立索引。
    2.字段值具有唯一性的可以建立唯一索引
    3.经常做where条件、join的字段、order by、group by 的字段，还有distinct，值得建立索引
    4.order by、group by同时存在的情况，适合建立联合索引
    5.高选择性（区分度）的字段适合建立索引，低选择性字段（比如性别、状态）不适合建立索引。区分度超过33%
    6.使用最频繁的列建议放在联合索引左侧
    7.如果有多个字段要创建索引，联合索引优于单值索引

    不适合建立索引的字段：
        需要频繁改动的字段，频繁更新索引会非常消耗性能（经常更新的表也不适合创建太多索引）
        很少被用于查询的字段（在where、group by、order by中都极少使用）
        数据量极小的表（<1万行)，全表扫描更快
